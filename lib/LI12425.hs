...
module LI12425 (
    ...
    Jogo(..), Onda(..), Loja,
    -- ** Outros
    Animacao(..), TipoInimigo(..), PosicaoIntermedia,
    -- * Funções auxiliares
    geraAleatorios
    ) where
...
-- | Posicao intermedia entre duas posicoes de um inimigo
type PosicaoIntermedia = (Posicao, Posicao, Float) 

-- | Animações possíveis
data Animacao 
  = -- | Animação a andar
    Walk 
  | -- | Animação quando está congelado
    Frozen
  deriving (Eq, Show)
...
data TipoInimigo
  = -- | Inimigo padrão
    Normal
  | -- | Inimigo agil mas com pouca vida e ataque
    Rapido
  | -- | Inimigo lento mas com muita vida e ataque
    Tanque
    deriving (Eq, Show)
...
data Inimigo = Inimigo
  { tipoInimigo :: TipoInimigo,
    ...
    animacao :: Animacao,
    -- | Frame atual da animação do inimigo
    frameInimigo :: Int,
    -- | Tempo da animação      
    tempoAInimigo :: Float,
    -- | Caminho a seguir até à torre    
    caminhoInimigo :: [Posicao],
    -- | Interpolacao
    posMedia :: Maybe PosicaoIntermedia
  } 
  deriving (Show, Eq)
